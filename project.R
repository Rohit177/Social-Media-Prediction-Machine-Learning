...
library(caret)
confusionMatrix(table(test$Influence_lifestyle, fitted.results))
...
#future data result
table(my_solution$Influence_lifestyle, my_prediction)


# Finish the write.csv() call
write.csv(my_solution, file = "my_solution_impact_logit.csv", row.names = FALSE)
...
#categorical variables
xtabs(~Comm_teacher,train)
ggplot(train) + geom_bar(aes(x=Comm_teacher))
ggplot(train) + geom_bar(aes(x=Gender))
ggplot(train) + geom_bar(aes(x= Preferences))

#numerical variables
ggplot(train) + geom_histogram(aes(x=Hours),fill = "white", colour = "black")
ggplot(train) + geom_histogram(aes(x=Age),fill = "white", colour = "black")
